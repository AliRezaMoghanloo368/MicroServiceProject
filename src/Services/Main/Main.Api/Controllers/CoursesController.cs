    ...
    public class CoursesController : GenericController
    {
        ...
        public async Task<ActionResult<CourseDto>> GetCourseById(long id)
        {
            ...
            if (course.Data != null)
            {
                ...
                var h = await _grpService.GetHistories("test", "course", recordId);
        ...
        public async Task<ActionResult<IReadOnlyList<CourseDto>>> GetCourses(long id)
        {
            ...
            #region GetHistory
            if (courses.Data != null && courses.Data.Count > 0)
            {
                int i = 0;
                foreach (var courseDto in courses.Data)
                {
                    var recordIds = courses.Data.Select(c => c.Id).ToList();

                    // gRPC call برای گرفتن همه histories
                    var h = await _grpService.GetHistories("test", "course", recordIds[i++].ToString());
        ...
        public async Task<ActionResult<int>> CreateCourse([FromBody] CreateCourseCommand command)
        {
            ...
            // For log
            await _grpService.CreateHistoryAsync("course", result.Data.Id.ToString(), HistoryAction.add);
        ...
        public async Task<ActionResult> UpdateCourse([FromBody] UpdateCourseCommand command)
        {
            ...
            // For log
            await _grpService.CreateHistoryAsync("course", command.Id.ToString(), HistoryAction.edit);
        ...
        public async Task<ActionResult> DeleteCourse(long id)
        {
            ...
            // For log
            await _grpService.CreateHistoryAsync("course", id.ToString(), HistoryAction.edit);
